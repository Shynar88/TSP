...
class GeneticAlgorithm():
    ...
    def generate_path(self):
        ...
        for generation in range(self.max_generations):
            ...
            write_log(generation, population_sorted[0].route_distance)
...
def write_log(generation, shortest_distance):
    LOG_FORMAT = "%(message)s - %(message)s"
    logging.basicConfig(filename = "gengen_logs.log",
                        level = logging.DEBUG,
                        format = LOG_FORMAT)
    logger = logging.getLogger()
    logger.info((generation, shortest_distance))
