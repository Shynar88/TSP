...
class GeneticAlgorithm():
    ...
    def crossover(self, p1, p2): # proposing good crossover method https://www.hindawi.com/journals/cin/2017/7430125/
        ...
        print(f"not_used_el_in_p2: {not_used_el_in_p2}")
        pointer = 0
        for _ in range(li):
            child.append(not_used_el_in_p2[pointer])
            pointer += 1
        print(f"child left: {child}")
        child += chunk
        print(f"child l + m: {child}")
        for _ in range(hi, len(p1)):
            child.append(not_used_el_in_p2[pointer])
            pointer += 1
        print(f"child final: {child}")
        return child
...
def main():
    ...
    gen_algo.crossover([1, 2, 3, 4, 5, 6, 7, 8, 9], [9, 8, 7, 6, 5, 4, 3, 2, 1])
